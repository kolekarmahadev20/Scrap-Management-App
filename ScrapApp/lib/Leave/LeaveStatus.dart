...
class _LeaveStatusState extends State<LeaveStatus> {
  ...
  void initState() {
    ...
    fetchLeaveData().then((data) {
      if (data != null) {
        setState(() {
          final sortedData = sortLatestToOld(data);
          leaveData = sortedData;
          filteredLeaveData = sortedData;
  ...
  void filterByLocation(String? location) {
    setState(() {
      ...
      if (location == null || location.isEmpty) {
        ...
        filteredLeaveData = leaveData
            .where((leave) => leave['location'] == location)
            .toList();
      }
    });
  }
  ...
  // ---------------- ADD IT HERE 👇 ----------------
  List<dynamic> sortLatestToOld(List<dynamic> data) {
    data.sort((a, b) {
      // 1️⃣ Sort by submitted_on date (latest first)
      DateTime submittedA =
          DateTime.tryParse(a['submitted_on'] ?? '') ?? DateTime(1970);
      DateTime submittedB =
          DateTime.tryParse(b['submitted_on'] ?? '') ?? DateTime(1970);

      int dateCompare = submittedB.compareTo(submittedA);
      if (dateCompare != 0) {
        return dateCompare;
      }

      // 2️⃣ If same submitted_on date → sort by ID (latest first)
          ...
          Padding(
            ...
            child: Row(
              children: [
                ...
                // 🔽 FILTER ICON WITH LOCATION LIST
                PopupMenuButton<String?>(
                  icon: const Icon(Icons.filter_alt),
                  onSelected: filterByLocation,
                  itemBuilder: (context) => [
